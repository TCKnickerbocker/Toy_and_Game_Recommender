...
import model_2_helper_functions
import model_2_alg
from model_2_configs import CONNECTION_PARAMS
import logging
import time
...
def call_model_2(user_id, num_recently_rated, num_recs_to_give, by_title=False):
    """
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            end_time0 = time.time()
            print(
                f"Getting connection and max_workers: {end_time0 - start_time:.2f} seconds"
            )
            # Use ThreadPoolExecutor for I/O bound operations
            most_similar_products = {}
            recently_rated_products = (
                model_2_helper_functions.get_recently_rated_products_info(
            ...
            with concurrent.futures.ThreadPoolExecutor(
                ...
                for rating_info in recently_rated_products:
                    ...
                    future = executor.submit(
                        model_2_helper_functions.get_n_most_similar_product_ids_model_2,
                        conn,
                        product_id,
                        n=num_recs_to_give,
                        similarity_tablename="product_title_similarity"
                        if by_title
                        else "product_description_similarity",
                        user_id=user_id,
                    )
            ...
            end_time2 = time.time()
            print(
                f"get_n_most_similar_product_ids_model_2 for {num_recently_rated} products took: {end_time2 - end_time1:.2f} seconds"
            )

            # Get products to recommend
            product_ids_to_rec = model_2_alg.recommend_products(
            ...
            x = model_2_helper_functions.get_products_by_product_ids(
                conn, product_ids_to_rec
            )
            end_time4 = time.time()
            print(
                f"get_products_by_product_ids took: {end_time4 - end_time3:.2f} seconds"
            )

            end_time5 = time.time()
            print(
                f"Total time for {num_recs_to_give} products with {num_recently_rated} recent ratings: {end_time5 - start_time:.2f} seconds"
            )
            return x

    except Exception as e:
        print(f"Error in call_model_2: {e}")
        raise


@app.route("/call_model_2", methods=["GET"])
